...
void Cnn(tapa::mmap<const float_v16> in_img, 
  ...
  tapa::task()
    .invoke(Mmap2Stream, in_img, in_img_stream, kNum*kInImSize*kInImSize)       
    .invoke(Mmap2Stream, weight, weight_stream, kNum*kNum*kKernel*kKernel)       
    .invoke(Mmap2Stream, bias, bias_stream, kNum)
    .invoke(Dummy, in_img_stream, weight_stream, bias_stream, out_img_stream, kNum, kKernel, kInImSize, kOutImSize)
    .invoke(Stream2Mmap, out_img_stream, out_img, kNum*kOutImSize*kOutImSize);
}
